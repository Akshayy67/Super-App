...
async function getTeamFiles(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (!teamData?.members?.includes(userId as string)) {
...
async function shareFile(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const teamData = teamDoc.data();
    if (!teamData?.members?.includes(sharedBy)) {
      return res.status(403).json({ error: 'Access denied: Not a team member' });
    }

    if (!teamData?.settings?.allowFileSharing) {
      return res.status(403).json({ error: 'File sharing is disabled for this team' });
    }
...
async function updateFilePermission(req: NextApiRequest, res: NextApiResponse) {
  const { userId, fileId, targetUserId, permission, action }: FilePermissionRequest & { targetUserId: string } = req.body;

  if (!userId || !fileId || !targetUserId || !permission || !action) {
    return res.status(400).json({ 
      error: 'User ID, file ID, target user ID, permission, and action are required' 
    });
  }
  ...
  try {
    ...
    if (!userPermissions.canManage) {
      return res.status(403).json({ error: 'Access denied: No admin permission' });
    }
