...
> = ({
  ...
  const [activeTab, setActiveTab] = useState<
    "overview" | "trends" | "skills" | "improvement"
  >("overview");
  ...
  const loadPerformanceData = () => {
    ...
    // If no real data exists, check if user wants to see sample data
    if (history.length === 0) {
      ...
      if (showSampleData) {
        ...
        setPerformanceHistory(sampleData);
      } else {
  ...
  const generateSamplePerformanceData = (): InterviewPerformanceData[] => {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      const baseScore = 65 + i * 3; // Scores improve from 65 to 77
      ...
      sampleData.push({
        ...
        overallScore: Math.round(
          Math.max(50, Math.min(95, baseScore + variance))
        ),
        technicalScore: Math.round(
          Math.max(50, Math.min(95, baseScore + variance + 5))
        ),
        communicationScore: Math.round(
          Math.max(50, Math.min(95, baseScore + variance - 2))
        ),
        behavioralScore: Math.round(
          Math.max(50, Math.min(95, baseScore + variance + 3))
        ),
        questionsAnswered: 6 + Math.floor(Math.random() * 4), // 6-9 questions
        questionsCorrect: Math.floor((6 + Math.random() * 4) * 0.7), // ~70% correct
        ...
        detailedMetrics: {
          ...
          clarity: Math.round(
            Math.max(50, Math.min(95, baseScore + variance + 1))
          ),
          professionalism: Math.round(
            Math.max(50, Math.min(95, baseScore + variance + 4))
          ),
        ...
        speechAnalysis: {
          ...
          confidenceScore: Math.max(
            0.6,
            Math.min(1.0, 0.65 + Math.random() * 0.35)
          ),
          emotionalTone: [
            "confident",
            "nervous",
  ...
  const skillBreakdownData = useMemo((): SkillBreakdown[] => {
    ...
    return [
      {
        skill: "Communication",
        current: currentPerformance.communicationScore,
        target: 90,
      ...
      },
      {
        ...
        current: currentPerformance.technicalScore,
        target: 85,
      ...
      },
      {
        ...
        current: currentPerformance.behavioralScore,
        target: 88,
      ...
      {
        skill: "Confidence",
        current: currentPerformance.detailedMetrics.confidence,
        target: 85,
      ...
      {
        skill: "Presentation",
        current: currentPerformance.detailedMetrics.professionalism,
        target: 90,
  ...
  const heatMapData = useMemo((): HeatMapData[] => {
    ...
    categories.forEach((category) => {
      difficulties.forEach((difficulty) => {
        const relevantInterviews = performanceHistory.filter(
          (p) =>
        ...
        const avgPerformance =
          relevantInterviews.length > 0
            ...
            : Math.random() * 40 + 50; // Fallback for demo
  ...
  }, [currentPerformance, skillBreakdownData]);
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto p-6">
          ...
          localStorage.getItem("show_sample_analytics_data") === "true" && (
            ...
            >
              ...
              <p className="text-sm mt-1">
                You're viewing sample analytics data. Complete a real interview
                to see your actual performance metrics.
              </p>
            </div>
          )}

        {/* Tab Content */}
        {performanceHistory.length > 0 && activeTab === "overview" && (
        ...
        {performanceHistory.length > 0 && activeTab === "trends" && (
          <div className="space-y-6">
            {/* Chart Type Selector */}
              ...
              <div className="grid grid-cols-3 gap-4">
                {heatMapData.map((item, index) => (
                  <div
                    ...
                    onClick={() =>
                      setShowDrillDown(
                        showDrillDown === `${item.category}-${item.difficulty}`
                          ? null
        ...
        {performanceHistory.length > 0 && activeTab === "skills" && (
          <div className="space-y-6">
        ...
        {performanceHistory.length > 0 && activeTab === "improvement" && (
          <div className="space-y-6">
            {/* Improvement Roadmap */}
            <div
              className={`rounded-lg shadow-lg p-6 ${
