...
export const AnalyticsDashboardIntegration: React.FC<AnalyticsDashboardIntegrationProps> = ({
  currentInterview,
  onStartNewInterview,
  onScheduleFollowUp,
}) => {
  const [currentPerformance, setCurrentPerformance] = useState<InterviewPerformanceData | undefined>(currentInterview);
  const [userPreferences, setUserPreferences] = useState<UserPreferences>(analyticsStorage.getUserPreferences());
  ...
  const generateSampleData = (): InterviewPerformanceData => {
    const now = new Date();
    return {
      id: `sample_${Date.now()}`,
    ...
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 dark:text-gray-300">Loading your analytics dashboard...</p>
        </div>
      </div>
    );
  }

  // Use sample data if no real data is available (for demonstration)
  const displayPerformance = currentPerformance || generateSampleData();

  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors duration-300">
      {/* Mobile-friendly responsive wrapper */}
      <div className={`${breakpoint === 'xs' || breakpoint === 'sm' ? 'px-2' : 'px-4'}`}>
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs opacity-90 mt-1">
                You're viewing sample data. Complete an interview to see your real analytics.
              </p>
              {onStartNewInterview && (
                <button
...
const StorageUsageWarning: React.FC = () => {
  const [storageUsage, setStorageUsage] = useState(analyticsStorage.getStorageUsage());
