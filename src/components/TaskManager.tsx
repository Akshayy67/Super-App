...
export const TaskManager: React.FC = () => {
  ...
  const loadTasks = () => {
    if (!user) return;
    const userTasks = storageUtils.getTasks(user.id);
    setTasks(userTasks.sort((a, b) => new Date(a.dueDate).getTime() - new Date(b.dueDate).getTime()));
  };
  ...
  const handleAddTask = () => {
    ...
    storageUtils.storeTask(newTask);
    resetForm();
  ...
  const handleEditTask = () => {
    ...
    storageUtils.updateTask(editingTask.id, updates);
    setEditingTask(null);
    resetForm();
    loadTasks();
  };
  ...
  const toggleTaskStatus = (task: Task) => {
    storageUtils.updateTask(task.id, {
